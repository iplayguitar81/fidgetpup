...
@php
  ...
  @endphp
      ...
      {{--<p>  {{strip_tags((str_limit($item->body, 100)))}}...</p>--}}
        ...
        <span class="count-icon" onclick="window.location='{{ route('posts.show', [$item->id, str_slug($item->title)]) }}/#fb-comments-show';"><i class="fa fa-2x fa-comment"></i> <span class="fb-comments-count" data-href="https://dev.fidgetspinnerdog.com/posts/{{$item->id}}/{{str_slug($item->title)}}"></span> Comments </span>
